...
Let's make some more changes. We will use the third Terraform file for this, so execute the comands below:
...
The output from ``diff`` shows we have added some extra lines from the pervious exercise:
...
This can be useful on its own, but the information could also be parsed and used in other areas of our Terraform logic, where we might choose to perform a tasks if the PAN-OS software is equal to or graeter than a certain version number.